...
from src.core.tasks import aws_analyse_image_task, ibm_analyse_image_task, google_analyse_image_task, azure_analyse_image_task, deep_ai_analyse_image_task, clarifai_analyse_image_task
...
class ClarifaiAnalyseImageTast(TestCase):

    def setUp(self):
        self.analysed_image = mommy.make(AnalysedImage, _create_files=True)

    @patch('src.core.tasks.analysers.clarifai_analyser')
    def test_populate_clarifai_result_with_the_output(self, mocked_analyser):
        mocked_analyser.return_value = {'some': 'data'}

        clarifai_analyse_image_task(self.analysed_image.id)
        self.analysed_image.refresh_from_db()

        assert self.analysed_image.clarifai_result == {'some': 'data'}
        mocked_analyser.assert_called_once_with(self.analysed_image.image.url)
    ...
    def test_do_not_update_if_no_result(self, mocked_analyser):
        ...
        assert self.analysed_image.clarifai_result == {}
        mocked_analyser.assert_called_once_with(self.analysed_image.image.url)

    @patch('src.core.tasks.analysers.clarifai_analyser')
    def test_fails_silently_if_no_image(self, mocked_analyser):
        self.analysed_image.delete()
        result = clarifai_analyse_image_task(self.analysed_image.id)
        assert result is None
